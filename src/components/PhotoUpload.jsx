...
const PhotoUpload = ({ selectedField, onClose, onAnalysisComplete }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const cropType = selectedField ? selectedField.crop : 'General';

            // Mock sensor data if not provided
            const sensorData = {
                weather: { temperature: 30, humidity: 60 },
                soil: { moisture: 40, soilType: 'Loamy', nitrogen: 100, phosphorus: 50, potassium: 50 }
            };

            const geminiResponse = await callGeminiImageAPI(base64Image, cropType, sensorData, i18n.language);

            setResult(geminiResponse);

            if (geminiResponse.success) {
                const mlResponse = await callMLPredictionAPI(geminiResponse, cropType, sensorData);
                if (mlResponse && mlResponse.predictions) {
                    setPredictions(mlResponse.predictions);
                }
                if (onAnalysisComplete) {
                    onAnalysisComplete(geminiResponse);
                }
            }
        } catch (error) {
            console.error('Analysis error:', error);
            setResult({ success: false, error: error.message || 'Failed to analyze image' });
        } finally {
